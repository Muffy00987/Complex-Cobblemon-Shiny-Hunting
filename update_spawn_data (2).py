...
def build_evolution_map():
    ...
    for r in pokemon_evolution:
        evolved_id=_int_or_none(r.get('evolved_species_id'))
        if evolved_id is None:
            continue
        evolved_row=next((s for s in species if int(s['id'])==evolved_id),None)
        if not evolved_row:
            continue
        from_id=_int_or_none(evolved_row.get('evolves_from_species_id'))
        if from_id is None:
            continue
        ...
        if not conds and trigger_name:
            conds.append(trigger_name)

        out.setdefault(from_id,[]).append({
            'to_species_id':evolved_id,
            'to_name':species_name_by_id.get(evolved_id,str(evolved_id)),
